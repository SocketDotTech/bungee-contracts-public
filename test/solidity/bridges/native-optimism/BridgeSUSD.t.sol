...
contract OptimismBridgeSUSDTest is Test, SocketGatewayBaseTest {
    ...
    address constant sender1 = 0xF977814e90dA44bFA03b6295A0616a897441aceC;
    ...
    function testSendSUSDOnL1BridgeRequest() public {
        address token = sUSD;
        address _customBridgeAddress = customSynthAddress;
        address _l2Token = optSusd;
        uint256 bridgingAmount = 1000e18;
        uint256 _interfaceId = synthInterfaceId;
        uint32 _l2Gas = 2000000;
        bytes memory _data = "0x";
        bytes memory eventData = abi.encodePacked("native-optimism", "SUSD");
        ...
        IERC20(sUSD).approve(address(socketGateway), bridgingAmount);

        uint256 SUSDBalance_BeforeBridging = IERC20(sUSD).balanceOf(sender1);

        assertEq(IERC20(sUSD).balanceOf(address(socketGateway)), 0);

        uint256 gasStockBeforeBridge = gasleft();

        socketGateway.executeRoute{value: 0}(513, impldata, eventData);

        uint256 gasStockAfterBridge = gasleft();

        //assert that SUSD balance of SocketGateway should be 0
        assertEq(IERC20(sUSD).balanceOf(address(socketGateway)), 0);

        //assert that SUSD balance of Sender should reduce by bridgedAmount
        uint256 SUSDBalance_AfterBridging = IERC20(sUSD).balanceOf(sender1);
        assertEq(
            SUSDBalance_BeforeBridging - SUSDBalance_AfterBridging,
            bridgingAmount
        );
