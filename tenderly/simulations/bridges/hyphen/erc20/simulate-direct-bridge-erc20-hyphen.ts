...
export const simulateDirectBridgeERC20OnHyphen = async (provider: any, forkBlockNumber: number, sourceChain: ChainCodes, destinationChain: ChainCodes, sender: string) => {
    ...
    try {
        ...
        await addERC20Allowance(provider, token!, sender, liquidityPoolManagerAddress, bridgingAmount);
        ...
        const bridgeTxn = await hyphenBridgeInstance.depositErc20(
            destinationChainId, token, recipient, bridgingAmount, tag,
            {
                gasLimit: 600000,
        ...
        if (!txnReceipt.status) {
            throw new Error("Hyphen ERC20-Direct-Bridge simulation failed");
        }
        ...
        if(parseInt(erc20BalanceAfterBridging) != 0){
            throw new Error("HyphenL1 DirectBride simulation failed - Assertion on ERC20 Balance after Bridging");
        }
