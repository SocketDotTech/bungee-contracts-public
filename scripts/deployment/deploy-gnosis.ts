...
const CONTRACT_NAME = "GnosisNativeBridgeImpl";
...
export const deployBridge = async () => {
  try {
    ...
    const gnosisXdaiBridge = bridgeConfig[networkName].gnosisXdaiBridge;
    const gnosisOmniBridge = bridgeConfig[networkName].gnosisOmniBridge;
    const gnosisWethOmniBridgeHelper =
      bridgeConfig[networkName].gnosisWethOmniBridgeHelper;

    if (!gnosisXdaiBridge || !gnosisOmniBridge || !gnosisWethOmniBridgeHelper) {
      throw new Error(
        "gnosisXdaiBridge // gnosisOmniBridge || gnosisWethOmniBridgeHelper not found"
      );
    }
