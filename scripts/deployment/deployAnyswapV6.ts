...
const CONTRACT_NAME = "AnyswapV6L2Impl";
...
export const deployBridge = async () => {
  try {
    ...
    const anyswap_router_v6 =
      bridgeConfig[networkName].anyswap_router_v6;

      const anyswap_router_v6_non_evm =
      bridgeConfig[networkName].anyswap_router_v6_non_evm;

    if (!anyswap_router_v6 || !anyswap_router_v6_non_evm )
      throw new Error(
        " anyswap_router_v6 address not found in config file"
      );
    ...
    console.log({ anyswap_router_v6 });
    const factory = await ethers.getContractFactory(CONTRACT_NAME);
    const Contract = await factory.deploy(
        anyswap_router_v6,
        anyswap_router_v6_non_evm,
      socketGateway,
      socketDeployFactory
    );
    ...
    await run("verify:verify", {
        ...
        constructorArguments: [anyswap_router_v6,anyswap_router_v6_non_evm, socketGateway, socketDeployFactory],
    });
