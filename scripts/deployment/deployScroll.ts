...
const CONTRACT_NAME = "ScrollImpl";
...
export const deployBridge = async () => {
  try {
    ...
    const _scrollL1GatewayRouter =
      bridgeConfig[networkName].scrollL1GatewayRouter;

    if (!_scrollL1GatewayRouter)
      throw new Error(
        " _scrollL1GatewayRouter address not found in config file"
      );
    ...
    console.log({ _scrollL1GatewayRouter });
    const factory = await ethers.getContractFactory(CONTRACT_NAME);
    const Contract = await factory.deploy(
      _scrollL1GatewayRouter,
      socketGateway,
      socketDeployFactory
    );
