...
const CONTRACT_NAME = "ZkSyncBridgeImpl";
...
export const deployBridge = async () => {
  try {
    ...
    const zksyncL1Mailbox = bridgeConfig[networkName].zkSyncEthRouter;
    const zkSyncERC20Bridge = bridgeConfig[networkName].zkSyncERC20Router;

    if (!zksyncL1Mailbox)
      throw new Error(" zksyncL1Mailbox address not found in config file");

    if (!zkSyncERC20Bridge)
      throw new Error(" zkSyncERC20Bridge address not found in config file");
    ...
    console.log({ zkSyncERC20Bridge, zksyncL1Mailbox });
    ...
    await run("verify:verify", {
      ...
      //   address: "0x2fa9fd7259a1634666916f5a597dfe99029dfc67",
