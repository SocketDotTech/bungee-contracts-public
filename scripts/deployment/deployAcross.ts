...
const CONTRACT_NAME = "AcrossImpl";
...
export const deployBridge = async () => {
  try {
    ...
    const spookPool = bridgeConfig[networkName].spookPool;
    const wethAddress = bridgeConfig[networkName].wethAddress;

    if (!spookPool || !wethAddress)
      throw new Error("spookPool or WETH address not found in config file");
    ...
    console.log({ spookPool, wethAddress });
