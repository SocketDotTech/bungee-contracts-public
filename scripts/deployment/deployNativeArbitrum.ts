...
const CONTRACT_NAME = "NativeArbitrumImpl";
...
export const deployBridge = async () => {
  try {
    ...
    const arbitrumRouter = bridgeConfig[networkName].arbitrumRouter;

    if (!arbitrumRouter)
      throw new Error(" arbitrumRouter address not found in config file");
