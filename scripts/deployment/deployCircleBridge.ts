...
export const deployBridge = async () => {
  try {
    ...
    const tokenMessenger = bridgeConfig[networkName].tokenMessenger;

    if (!tokenMessenger)
      throw new Error(" tokenMessenger address not found in config file");
    ...
    console.log({ tokenMessenger });
    const factory = await ethers.getContractFactory(CONTRACT_NAME);
    const Contract = await factory.deploy(
      tokenMessenger,
      "0xc91E5068968ACAEC9C8E7C056390d9e3CB34f7FC",
      socketGateway,
      socketDeployFactory
    );
