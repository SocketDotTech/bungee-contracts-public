...
const CONTRACT_NAME = "RainbowSwapImpl";
...
export const deployRoute = async () => {
  try {
    ...
    //   console.log("RainbowSwapImpl already deployed at ", deployment?.[CONTRACT_NAME]);
    //   return {
    //     success: true,
    ...
    const rainbowSwapAggregator =
      bridgeConfig[networkName].rainbowSwapAggregator;

    if (!rainbowSwapAggregator)
      throw new Error("rainbowSwapAggregator not found in config");

    console.log("deployer ", deployer);
    console.log("owner", owner);
    console.log("socketGateway", socketGateway);
    console.log("socketDeployFactory", socketDeployFactory);
    console.log("rainbowSwapAggregator", rainbowSwapAggregator);
