...
const CONTRACT_NAME = "SymbiosisBridgeImpl";
...
export const deployBridge = async () => {
  try {
    ...
    const symbiosisMetaRouter = bridgeConfig[networkName].symbiosisMetaRouter;
    const symbiosisMetaRouterGateway =
      bridgeConfig[networkName].symbiosisMetaRouterGateway;

    if (!symbiosisMetaRouter || !symbiosisMetaRouterGateway) {
      throw new Error(
        "symbiosisMetaRouter // symbiosisMetaRouterGateway not found"
      );
    }
    ...
    console.log({ symbiosisMetaRouter, symbiosisMetaRouterGateway });

    const factory = await ethers.getContractFactory(CONTRACT_NAME);
    const Contract = await factory.deploy(
      symbiosisMetaRouter,
      symbiosisMetaRouterGateway,
      socketGateway,
      socketDeployFactory
    );
