...
const CONTRACT_NAME = "AnyswapL2Impl";
...
export const deployBridge = async () => {
  try {
    ...
    const anyswap_router_v4 =
      bridgeConfig[networkName].anyswap_router_v4;

    if (!anyswap_router_v4 )
      throw new Error(
        " anyswap_router_v4 address not found in config file"
      );
    ...
    await run("verify:verify", {
        ...
        constructorArguments: [anyswap_router_v4, socketGateway, socketDeployFactory],
    });
