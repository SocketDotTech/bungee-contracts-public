...
export const deployRoute = async () => {
  try {
    ...
    if (deployment?.[CONTRACT_NAME]) {
      console.log("ZeroXSwapImpl already deployed at ", deployment?.[CONTRACT_NAME]);
      return {
        success: true,
    ...
    const zeroXExchangeProxy = bridgeConfig[networkName].zeroXExchangeProxy ;

    if(!zeroXExchangeProxy) throw new Error("zeroXExchangeProxy not found in config");
    ...
    console.log("zeroXExchangeProxy", zeroXExchangeProxy);

    const factory = await ethers.getContractFactory(CONTRACT_NAME);
    const Contract = await factory.deploy( zeroXExchangeProxy,socketGateway, socketDeployFactory);
    ...
    await run("verify:verify", {
      ...
      constructorArguments: [zeroXExchangeProxy,socketGateway, socketDeployFactory],
    });
