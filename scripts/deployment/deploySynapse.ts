...
const CONTRACT_NAME = "SynapseBridgeImpl";
...
export const deployBridge = async () => {
  try {
    ...
    const synapseRouter =
      bridgeConfig[networkName].synapseRouter;

    if (!synapseRouter )
      throw new Error(
        " liquidityPoolManager address not found in config file"
      );
    ...
    await run("verify:verify", {
      ...
      constructorArguments: [synapseRouter, socketGateway, socketDeployFactory],
    });
