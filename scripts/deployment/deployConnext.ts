...
const CONTRACT_NAME = "ConnextImpl";
...
export const deployBridge = async () => {
  try {
    ...
    const connextRouter = bridgeConfig[networkName].connextRouter;
    const wethAddress = bridgeConfig[networkName].wrappedTokenAddress;

    if (!connextRouter || !wethAddress)
      throw new Error("connextRouter or WETH address not found in config file");
    ...
    console.log({ connextRouter, wethAddress });
